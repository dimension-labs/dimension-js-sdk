...
import { DimensionClient } from '../../src/lib/DimensionClient';
...
describe('DimensionClient', () => {
  before(() => {
    dimensionClient = new DimensionClient('http://192.168.2.166:40101/rpc');
  });

  it('should generate new Ed25519 key pair, and compute public key from private key', () => {
    const edKeyPair = dimensionClient.newKeyPair(SignatureAlgorithm.Ed25519);
    const publicKey = edKeyPair.publicKey.value();
    const privateKey = edKeyPair.privateKey;
    const convertFromPrivateKey = dimensionClient.privateToPublicKey(
      privateKey,
  ...
  it('should generate PEM file for Ed25519 correctly', () => {
    ...
    const publicKeyFromFIle = dimensionClient.loadPublicKeyFromFile(
      tempDir + '/public.pem',
      SignatureAlgorithm.Ed25519
    );
    const privateKeyFromFile = dimensionClient.loadPrivateKeyFromFile(
      tempDir + '/private.pem',
      SignatureAlgorithm.Ed25519
    );
    ...
    const loadedKeyPair = dimensionClient.loadKeyPairFromPrivateFile(
      tempDir + '/private.pem',
      SignatureAlgorithm.Ed25519
    );
  ...
  it('should generate new Secp256K1 key pair, and compute public key from private key', () => {
    const edKeyPair = dimensionClient.newKeyPair(SignatureAlgorithm.Secp256K1);
    const publicKey = edKeyPair.publicKey.value();
    const privateKey = edKeyPair.privateKey;
    const convertFromPrivateKey = dimensionClient.privateToPublicKey(
      privateKey,
  ...
  it('should generate PEM file for Secp256K1 and restore the key pair from PEM file correctly', () => {
    const edKeyPair: Secp256K1 = dimensionClient.newKeyPair(
      SignatureAlgorithm.Secp256K1
    );
    const publicKeyInPem = edKeyPair.exportPublicKeyInPem();
    const privateKeyInPem = edKeyPair.exportPrivateKeyInPem();
  ...
  it('should create a HK wallet and derive child account correctly', function() {
    ...
    const hdKey = dimensionClient.newHdWallet(decodeBase16(seed));
  ...
  it('should create deploy from Deploy JSON with ttl in minutes', function() {
    ...
    const fromJson = dimensionClient.deployFromJson(json).unwrap();
