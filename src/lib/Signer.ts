/**
 * Provide methods to communicate with DimensionLabs Signer Extension.
...
import {
  DimensionLabsHelper,
  SignerTestingHelper
} from '../@types/dimensionlabsSigner';

declare global {
  interface Window {
    dimensionlabsHelper: DimensionLabsHelper;
    signerTestingHelper: SignerTestingHelper;
  }
}

const helperPresent = () => {
  return !(typeof window.dimensionlabsHelper === 'undefined');
};
...
export const getVersion: () => Promise<string> = async () => {
  if (helperPresent()) {
    try {
      return await window.dimensionlabsHelper.getVersion();
    } catch {
      return '<1.0.0';
    }
  }
...
/**
 * Check whether DimensionLabs Signer extension is connected
 */
export const isConnected: () => Promise<boolean> = async () => {
  if (helperPresent()) return await window.dimensionlabsHelper.isConnected();
  return Promise.reject(
    new Error(
      'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
    )
...
export const sendConnectionRequest: () => void = () => {
  if (helperPresent()) return window.dimensionlabsHelper.requestConnection();
  throw new Error(
    'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
  );
};
 ...
 *
 * @throws Error if haven't connected to DimensionLabs Signer browser extension.
 */
export const getSelectedPublicKeyBase64: () => Promise<string> = () => {
  if (helperPresent())
    return window.dimensionlabsHelper.getSelectedPublicKeyBase64();
  throw new Error(
    'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
  );
};
...
export const getActivePublicKey: () => Promise<string> = () => {
  if (helperPresent()) return window.dimensionlabsHelper.getActivePublicKey();
  return Promise.reject(
    new Error(
      'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
    )
...
export const sign: (
  ...
  if (helperPresent())
    return window.dimensionlabsHelper.sign(
      deploy,
      signingPublicKeyHex,
      targetPublicKeyHex
    );
...
export const signMessage: (
  ...
  if (helperPresent())
    return window.dimensionlabsHelper.signMessage(message, signingPublicKey);
  return Promise.reject(
    new Error(
      'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
    )
...
export const disconnectFromSite: () => void = () => {
  if (helperPresent()) return window.dimensionlabsHelper.disconnectFromSite();
  return Promise.reject(
    new Error(
      'Content script not found - make sure you have the Signer installed and refresh the page before trying again.'
    )
